...
(async () => {
  ...
  // 카운트다운 숫자가 실제로 3->2->1로 바뀌는지 확인 (호스트 화면 기준)
  const seenNumbers = [];
  const cdStart = Date.now();
  while (Date.now() - cdStart < 6000) {
    const overlayVisible = await page1.evaluate(() => !document.getElementById("overlay-countdown").classList.contains("hidden"));
    if (overlayVisible) {
      const num = await page1.textContent("#overlay-countdown-num");
      if (seenNumbers[seenNumbers.length - 1] !== num) seenNumbers.push(num);
    }
    await page1.waitForTimeout(100);
  }
  console.log("카운트다운 중 관찰된 숫자 순서:", seenNumbers);
  const countdownOk = seenNumbers.join(",") === "3,2,1" || seenNumbers.join(",") === "3,2,1,GO!";
  console.log("카운트다운 정상 여부(3→2→1):", countdownOk);
  ...
  await browser.close();
  process.exit(
    cards.length === 2 && cards2.length === 2 && err1.length === 0 && err2.length === 0 && canvasHasContent1 && canvasHasContent2 && countdownOk
      ? 0
      : 1
  );
})();
